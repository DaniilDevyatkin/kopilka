...
- существующий nginx обслуживает HTTPS через встроенный Certbot webroot без дополнительного nginx-плагина; native Caddy используется только как fallback на чистом VDS и полностью удаляется после успешного перехода на nginx;
